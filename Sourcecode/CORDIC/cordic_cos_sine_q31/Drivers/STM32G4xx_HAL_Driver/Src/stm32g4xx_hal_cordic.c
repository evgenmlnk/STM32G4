...
extern uint16_t hrtim_exe_time;
                                       ...
                                       uint32_t NbCalc, uint32_t Timeout)
{
  ...
  if (hcordic->State == HAL_CORDIC_STATE_READY)
  {
    ...
    start = (uint16_t) hhrtim1.Instance->sTimerxRegs->CNTxR;
    ...
    /* Read output data from Read Data register, and increment output buffer pointer */
    CORDIC_ReadOutDataIncrementPtr(hcordic, &p_tmp_out_buff);
    hrtim_exe_time = (uint16_t) (hhrtim1.Instance->sTimerxRegs->CNTxR - start);
                                         ...
                                         uint32_t NbCalc, uint32_t Timeout)
{
  ...
  if (hcordic->State == HAL_CORDIC_STATE_READY)
  {
    ...
    /* Write of input data in Write Data register, and increment input buffer pointer */
    start_time = (uint16_t) hhrtim1.Instance->sTimerxRegs->CNTxR;
    CORDIC_WriteInDataIncrementPtr(hcordic, &p_tmp_in_buff);
...
//      CORDIC_ReadOutDataIncrementPtr(hcordic, &p_tmp_out_buff);
//      //hrtim_exe_time = hhrtim1.Instance->sTimerxRegs->CNTxR - start_hrtime;
//      execution_time= htim2.Instance->CNT - start_time;
//
//
//       Check for the Timeout
//      if (Timeout != HAL_MAX_DELAY)
...
//    }
    ...
    if(end < start)
    {
    	 hrtim_exe_time=(TIMER_PERIOD -start_time)+ end_time +1;
    }
    else
    {
    	hrtim_exe_time= end_time-start_time;
    }

    //HAL_HRTIM_SimpleBaseStop(&hhrtim1, HRTIM_TIMERINDEX_TIMER_A);
    // execution_time= htim2.Instance->CNT - start_time;
    /* Change the CORDIC state */
    hcordic->State = HAL_CORDIC_STATE_READY;
