...
uint32_t frameCout;
volatile uint32_t aDWTCycleCount[NO_OF_FRAMES];
  ...
  CurrentInputArraySize = BLOCK_SIZE;
   ...
   currentFrame++;
	  ...
	  do {
		  ...
		  aDWTCycleCount[currentFrame-1]  = dwtCycleCountStop - dwtCycleCountStart;
				 ...
				 currentFrame++;
				  ...
				  /* end of loop */
		  } while(OutputDataReadyCallbackCount < NO_OF_FRAMES);
		  {
		    if(ErrorCount != 0)
		    {
		      /* Processing Error */
