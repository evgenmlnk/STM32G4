...
uint16_t CurrentInputArraySize = BLOCK_SIZE;

static int16_t aCalculatedFilteredData_q15[BLOCK_SIZE];

uint16_t ExpectedCalculatedFilteredDataSize = BLOCK_SIZE;
...
uint32_t dwtCycleCountStart;
uint32_t dwtCycleCountStop;
volatile uint32_t dwtCycleCount;
uint32_t frameCout;
uint32_t aDWTCycleCount[NO_OF_FRAMES];
...
int main(void)
{
  ...
  /*## Start calculation of FIR filter in polling mode ####################*/
  ...
  CurrentInputArraySize = BLOCK_SIZE;
	  ...
	  do {
		  ...
		  if (HAL_FMAC_AppendFilterData(&hfmac,
		    ...
		    /* While processing is going on, fill next frame from Flash */

			for (i = 0; i<BLOCK_SIZE; i++)
			{

			  aInputValues_q15[i] = aInputSignal_q15[i+BLOCK_SIZE*currentFrame];
			}



		    /*## Poll to write input data ##########################*/
		    ...
		    aDWTCycleCount[currentFrame] = dwtCycleCountStop - dwtCycleCountStart;


		    if (HAL_FMAC_ConfigFilterOutputBuffer(&hfmac,
		    						aCalculatedFilteredData_q15,
...
//		  {
//
//			  printf("%d, ", aCalculatedFilteredData_q15[i]);
//		  }
